...
uint8_t EEPROM24CXX_Internal::getDeviceAddress(uint32_t address)
{
    uint8_t dev = deviceAddress;
...
bool EEPROM24CXX_Internal::checkAddress(uint32_t address, uint32_t length)
{
...
bool EEPROM24CXX_Internal::waitReady(uint32_t timeout)
{
...
bool EEPROM24CXX_Internal::writeByteRaw(uint32_t address, uint8_t value)
{
    if(!checkAddress(address))
        return false;

    uint8_t dev = getDeviceAddress(address);
...
uint8_t EEPROM24CXX_Internal::readByteRaw(uint32_t address)
{
    if(!checkAddress(address))
        return 0xFF;
...
)
{
    ...
    EEPROM24CXX_Internal::deviceAddress = i2cAddress;
    EEPROM24CXX_Internal::writeProtectPin = wpPin;
...
{
    return EEPROM24CXX_Internal::writeByteRaw(address, value);
}

uint8_t readByte(uint32_t address)
{
    return EEPROM24CXX_Internal::readByteRaw(address);
}
...
void lock_mem()
{
    if(EEPROM24CXX_Internal::writeProtectPin != 255)
    {
        digitalWrite(
...
{
    uint32_t pos = EEPROM24CXX_Internal::cursor + bytes;
...
{
    return EEPROM24CXX_Internal::cursor + bytes <=
...
void read_all()
{
    ...
    for(uint32_t i = 0;
        i < EEPROM24CXX_Internal::memorySize;
        i++)
    {
        if(i % 16 == 0)
        {
